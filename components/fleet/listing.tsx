...
import type { Jet, Media } from '@/payload-types'
...
const Fleet = ({data, images}:{data: Jet, images:{
    image: number | Media
    id?: string | null
  }[]}) => {
  ...
  return (
    <section key={data.id} className={'w-full flex justify-center items-center my-[8vh]'}>
      <div
        className={
        ...
        >
          <div className={'flex flex-col justify-start items-start gap-[2vh]'}>
            <div className={'flex flex-col justify-start items-start gap-[0.25vh]'}>
              <h2 className={'cursor-pointer'}>{data.manufacturer}</h2>
              <h2 className={'cursor-pointer'}>{data.name}</h2>
            </div>
            <h2 className={'font-normal'}>{data.plate}</h2>
          </div>
          <p className={'text-justify cursor-pointer'}>{data.introduction}</p>
...
const Listing = ({ data }: { data: PaginatedDocs<Jet> }) => (
    <section className={'w-full flex flex-col justify-start items-center'}>
      {data.docs.map((doc) => (<Fleet key={doc.id} data={doc} images={[{ image: doc.images.hero }, ...doc.images.gallery]} />))}
    </section>
  )
